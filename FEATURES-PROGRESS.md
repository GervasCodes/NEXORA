...
**Decision update:** KYC tiers and `accountVerification` were originally two
parallel systems (see refactor note above). Roadmap decision: merge into one
flow. `AdminAccountVerifications.jsx` already consolidates the admin side;
`SellerVerification.jsx` as a standalone page is gone. Backend still has two
separate modules (`kyc/`, `accountVerification/`) — full merge is tracked as
follow-up work, not done as part of this note.
...
**Decision update:** referral and affiliate were reviewed as a possible
consolidation. Roadmap decision: keep both live simultaneously — they serve
different growth channels (referral = peer-to-peer/organic, affiliate =
commission-driven partners/influencers) and there's no usage/payout data
yet justifying sunsetting either.
...
**Decision update:** confirmed backlog, not reprioritized — the existing PWA
is sufficient for now; Capacitor/React Native stays deprioritized until the
PWA is shown to be insufficient.
