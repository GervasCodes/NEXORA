const db = require("../../config/db");
const featuredStoreRepository = require("./featuredStore.repository");
const productRepository = require("../product/product.repository");
const categoryRepository = require("../category/category.repository");
const walletRepository = require("../wallet/wallet.repository");
const settingsService = require("../settings/settings.service");
const notificationService = require("../notification/notification.service");

// Same bounds as sponsorship.service.js (Phase 8A) and for the same
// reason: long enough to be useful, short enough that a mistaken
// purchase can't lock up a large chunk of a seller's wallet for very
// long. Nothing stops them creating a new campaign the moment one ends.
const MIN_DAYS = 1;
const MAX_DAYS = 30;

const MS_PER_DAY = 24 * 60 * 60 * 1000;

// What a seller sees before committing to a campaign - the form on
// SellerFeaturedStore.jsx reads this to show "X/day" and compute a live
// total as they change the duration, same shape
// sponsorship.service.js#getPricing already uses.
exports.getPricing = async () => {
    const dailyRate = await settingsService.getFeaturedStoreDailyRate();
    return { daily_rate: dailyRate, min_days: MIN_DAYS, max_days: MAX_DAYS };
};

// Departments this seller can actually pay to be featured in - only
// ones they have at least one active product listed under (see
// product.repository.js#findActiveCategoriesBySeller).
exports.getEligibleCategories = async (sellerId) => {
    return productRepository.findActiveCategoriesBySeller(sellerId);
};

// Charges the seller's wallet, snapshots the rate that applied, and
// opens the campaign - all inside one transaction, same shape
// sponsorship.service.js#createCampaign (Phase 8A) already uses for
// "row-lock wallet, check funds, debit, write ledger entry". Unlike that
// phase, there is no products.is_sponsored-equivalent flag to flip here:
// featuredStore.repository.js's ranking join
// (category.repository.js#findFeaturedStoresByCategory) reads this
// table live, so a campaign takes effect and clears itself out purely
// by its own `status`/`ends_at` - nothing else needs to change in sync.
exports.createCampaign = async (sellerId, categoryId, days) => {
    const parsedDays = Number(days);
    if (!Number.isInteger(parsedDays) || parsedDays < MIN_DAYS || parsedDays > MAX_DAYS) {
        throw new Error(`Choose a duration between ${MIN_DAYS} and ${MAX_DAYS} days`);
    }

    const category = await categoryRepository.findById(categoryId);
    if (!category || !category.is_active) {
        throw new Error("Department not found");
    }

    const eligibleCategories = await productRepository.findActiveCategoriesBySeller(sellerId);
    if (!eligibleCategories.some((c) => c.id === Number(categoryId))) {
        throw new Error("You need an active, published product in this department before you can be featured there");
    }

    // A seller paying for a second campaign in a department that's
    // already actively featuring them would just be wasted spend - the
    // ranking join can't rank the same store twice. Blocked here rather
    // than left to silently overlap, unlike sponsorship_campaigns
    // (Phase 8A) which allows overlapping product campaigns since each
    // one is independently useful there.
    const alreadyActive = await featuredStoreRepository.hasActiveForSellerCategory(sellerId, categoryId);
    if (alreadyActive) {
        throw new Error("You already have an active featured-store campaign for this department");
    }

    const dailyRate = await settingsService.getFeaturedStoreDailyRate();
    const sponsorshipEnabled = await settingsService.isSponsorshipMonetizationEnabled();
    const totalCost = sponsorshipEnabled ? Number((dailyRate * parsedDays).toFixed(2)) : 0;

    const connection = await db.getConnection();

    try {
        await connection.beginTransaction();

        await walletRepository.ensureWallet(sellerId, connection);
        const wallet = await walletRepository.getWalletForUpdate(sellerId, connection);

        // Monetization Master Switch: while sponsorship monetization is
        // off, campaigns are free - skip the balance check and debit
        // entirely (totalCost is already 0 above) and auto-approve.
        if (sponsorshipEnabled && totalCost > Number(wallet.balance)) {
            throw new Error(
                "Insufficient wallet balance to fund this campaign. Top up from your order earnings, or choose a shorter duration."
            );
        }

        const endsAt = new Date(Date.now() + parsedDays * MS_PER_DAY);

        const campaignId = await featuredStoreRepository.create(
            { sellerId, categoryId, dailyRate: sponsorshipEnabled ? dailyRate : 0, days: parsedDays, totalCost, endsAt },
            connection
        );

        let balanceAfter = Number(wallet.balance);
        if (sponsorshipEnabled && totalCost > 0) {
            balanceAfter = await walletRepository.incrementBalance(sellerId, -totalCost, connection);

            await walletRepository.insertTransaction({
                sellerId,
                type: "debit",
                amount: totalCost,
                balanceAfter,
                referenceType: "featured_store_campaign",
                referenceId: campaignId,
                description: `Featured store campaign #${campaignId} in "${category.name}" (${parsedDays} day${parsedDays === 1 ? "" : "s"} at ${dailyRate}/day)`
            }, connection);
        }

        await connection.commit();

        notificationService.notify({
            userId: sellerId,
            type: "featured_store_started",
            titleKey: "notifications.featuredStore.started.title",
            messageKey: "notifications.featuredStore.started.message",
            messageParams: { categoryName: category.name, days: parsedDays, amount: totalCost },
            withEmail: false
        }).catch((err) => console.error("featured store start notify error:", err));

        return { campaignId, totalCost, balance: balanceAfter, endsAt };

    } catch (error) {
        await connection.rollback();
        throw error;

    } finally {
        connection.release();
    }
};

exports.getMyCampaigns = async (sellerId) => {
    return featuredStoreRepository.findBySeller(sellerId);
};

// Ends a still-running campaign early. Deliberately no pro-rated refund -
// same policy as sponsorship.service.js#cancelCampaign (Phase 8A) for
// the same reason (see README-phase-8A.md's "Not in scope" section).
exports.cancelCampaign = async (sellerId, campaignId) => {
    const connection = await db.getConnection();

    try {
        await connection.beginTransaction();

        const campaign = await featuredStoreRepository.findByIdForUpdate(campaignId, connection);

        if (!campaign || campaign.seller_id !== sellerId) {
            throw new Error("Campaign not found");
        }
        if (campaign.status !== "active") {
            throw new Error(`This campaign is already "${campaign.status}"`);
        }

        await featuredStoreRepository.updateStatus(campaignId, "cancelled", connection);

        await connection.commit();
        return { status: "cancelled" };

    } catch (error) {
        await connection.rollback();
        throw error;

    } finally {
        connection.release();
    }
};

// --- Cron job entry point (jobs/featuredStoreExpiry.job.js) -------------
//
// Closes out every campaign whose ends_at has passed. Idempotent: only
// ever touches rows still marked 'active', so it's safe to run on every
// tick even if the previous run already handled everything. Unlike
// sponsorship.service.js#expireDueCampaigns, there's no display flag to
// clear alongside the status - the ranking join reads `status`/`ends_at`
// directly, so flipping the status here is the whole effect.
exports.expireDueCampaigns = async () => {
    const connection = await db.getConnection();

    try {
        await connection.beginTransaction();

        const due = await featuredStoreRepository.findExpiredActive(connection);

        for (const campaign of due) {
            await featuredStoreRepository.updateStatus(campaign.id, "expired", connection);
        }

        await connection.commit();

        for (const campaign of due) {
            notificationService.notify({
                userId: campaign.seller_id,
                type: "featured_store_expired",
                titleKey: "notifications.featuredStore.expired.title",
                messageKey: "notifications.featuredStore.expired.message",
                messageParams: { categoryName: campaign.category_name },
                withEmail: false
            }).catch((err) => console.error("featured store expiry notify error:", err));
        }

        return due.length;

    } catch (error) {
        await connection.rollback();
        throw error;

    } finally {
        connection.release();
    }
};

// --- Admin oversight (read-only) -----------------------------------------
exports.listAllCampaigns = async () => {
    return featuredStoreRepository.findAll();
};
