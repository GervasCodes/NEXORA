-- Migration 109: user map location (Phase 5 - "map showing users")
-- Run after 108_ai_call_outcomes.sql.
--
-- Renumbered from its original 105 (collided with the unrelated
-- 105_seller_promo_video.sql - two feature branches both incremented
-- off 104 independently). No content change from the renumbering
-- itself.
--
...
--
-- Named location_lat_updated_at, NOT location_updated_at: migration 015
-- already put a location_updated_at column on `users`, paired with that
-- feature's own current_lat/current_lng (delivery-agent live tracking).
-- Reusing the name here would either collide (it does - this is why the
-- original migration failed) or, if silently reused, conflate two
-- different positions' timestamps under one column. Call sites alias it
-- back to location_updated_at in query results where useful, so the API
-- response shape for this feature is unaffected.
ALTER TABLE users
    ...
    ADD COLUMN location_lat_updated_at TIMESTAMP NULL AFTER location_lng,
    ADD COLUMN location_sharing_enabled TINYINT(1) NOT NULL DEFAULT 1 AFTER location_lat_updated_at;
