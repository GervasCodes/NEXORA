            ...
            {showForm && (
                <form onSubmit={submitIncident} className="border border-line rounded-lg p-4 mb-6 space-y-3 max-w-lg">
                    ...
                    <div className="flex gap-3">
                        <select value={component} onChange={(e) => setComponent(e.target.value)} className="flex-1 border border-line rounded-md px-3 py-2 text-sm">
                            {COMPONENTS.map((c) => <option key={c} value={c}>{c}</option>)}
                        </select>
                        <select value={severity} onChange={(e) => setSeverity(e.target.value)} className="flex-1 border border-line rounded-md px-3 py-2 text-sm">
