...
describe("ReadReceipt", () => {
    it("renders a single check (one path) for 'sent'", () => {
        ...
        expect(el.className).toContain("text-frost/55");
    });
    ...
    it("renders a double check in the bright 'read' color for 'read'", () => {
        render(<ReadReceipt status="read" />);
        const el = screen.getByLabelText("read");
        expect(el.querySelectorAll("path")).toHaveLength(2);
        expect(el.className).toContain("text-frost");
        expect(el.className).not.toContain("text-frost/55");
    });
});
